...
def require_user_api_key():
    return get_bool_config("REQUIRE_USER_API_KEY", False)

def normalize_api_keys(value):
    if not value:
        return []
    if isinstance(value, (list, tuple)):
        return [str(k).strip().strip("'\"") for k in value if str(k).strip()]
    return [k.strip().strip("'\"") for k in str(value).split(",") if k.strip()]
            ...
            with open(env_path, "r", encoding="utf-8") as f:
                for line in f:
                    if line.strip() and not line.strip().startswith("#"):
                        ...
                        if len(parts) == 2:
                            name, val = parts[0].strip().lstrip("\ufeff"), parts[1].strip()
                            if name in ["GEMINI_API_KEYS", "GEMINI_API_KEY", "GOOGLE_API_KEY"]:
                                keys.extend(normalize_api_keys(val))
        except Exception:
            pass
    return keys

def get_env_file_values(names):
    values = []
    env_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".env")
    if os.path.exists(env_path):
        try:
                ...
                for line in f:
                    if line.strip() and not line.strip().startswith("#"):
                        ...
                        if len(parts) == 2:
                            name, val = parts[0].strip().lstrip("\ufeff"), parts[1].strip()
                            if name in names:
                                values.extend(normalize_api_keys(val))
        except Exception:
            pass
    return values

def get_api_keys():
    # 1. Chave manual da sessão sempre tem prioridade.
    ...
    keys = []
    keys.extend(normalize_api_keys(os.environ.get("GEMINI_API_KEYS")))
    keys.extend(normalize_api_keys(os.environ.get("GEMINI_API_KEY")))
    keys.extend(normalize_api_keys(os.environ.get("GOOGLE_API_KEY")))
    keys.extend(normalize_api_keys(get_secret_value("GEMINI_API_KEYS")))
    keys.extend(normalize_api_keys(get_secret_value("GEMINI_API_KEY")))
    keys.extend(normalize_api_keys(get_secret_value("GOOGLE_API_KEY")))
    keys.extend(get_env_file_keys())
...
def get_groq_api_keys():
    if "groq_api_key_override" in st.session_state and st.session_state.groq_api_key_override:
        return [st.session_state.groq_api_key_override]

    keys = []
    keys.extend(normalize_api_keys(os.environ.get("GROQ_API_KEYS")))
    keys.extend(normalize_api_keys(os.environ.get("GROQ_API_KEY")))
    keys.extend(normalize_api_keys(get_secret_value("GROQ_API_KEYS")))
    keys.extend(normalize_api_keys(get_secret_value("GROQ_API_KEY")))
    keys.extend(get_env_file_values(["GROQ_API_KEYS", "GROQ_API_KEY"]))
...
GROQ_DEFAULT_MODEL = os.environ.get("GROQ_MODEL", str(get_secret_value("GROQ_MODEL") or "llama-3.1-8b-instant"))
GROQ_API_URL = "https://api.groq.com/openai/v1/chat/completions"
GROQ_TIMEOUT_SECONDS = int(os.environ.get("GROQ_TIMEOUT_SECONDS", "45"))
...
def mock_tenses(person):
    subject_map = {
        "I": ("I", "I", "eu", "practice", "do", "am", "have"),
        "You": ("You", "you", "você", "practice", "do", "are", "have"),
        "He": ("He", "he", "ele", "practices", "does", "is", "has"),
        "She": ("She", "she", "ela", "practices", "does", "is", "has"),
        "It": ("It", "it", "isso", "practices", "does", "is", "has"),
        "We": ("We", "we", "nós", "practice", "do", "are", "have"),
        "You (plural)": ("You", "you", "vocês", "practice", "do", "are", "have"),
        "They": ("They", "they", "eles/elas", "practice", "do", "are", "have"),
    }
    subject, question_subject, subject_pt, present_form, auxiliary, be_form, have_form = subject_map.get(
        person, (person, person.lower(), person, "practice", "do", "are", "have")
    )
    templates = [
        ("Present Simple", f"{subject} {present_form} English.", f"{auxiliary.capitalize()} {question_subject} practice English?", f"{subject} {auxiliary} not practice English.", f"{subject_pt.capitalize()} pratica inglês.", f"{subject_pt.capitalize()} pratica inglês?", f"{subject_pt.capitalize()} não pratica inglês."),
        ("Present Continuous", f"{subject} {be_form} practicing English.", f"{be_form.capitalize()} {question_subject} practicing English?", f"{subject} {be_form} not practicing English.", f"{subject_pt.capitalize()} está praticando inglês.", f"{subject_pt.capitalize()} está praticando inglês?", f"{subject_pt.capitalize()} não está praticando inglês."),
        ("Present Perfect", f"{subject} {have_form} practiced English.", f"{have_form.capitalize()} {question_subject} practiced English?", f"{subject} {have_form} not practiced English.", f"{subject_pt.capitalize()} tem praticado inglês.", f"{subject_pt.capitalize()} tem praticado inglês?", f"{subject_pt.capitalize()} não tem praticado inglês."),
        ("Present Perfect Continuous", f"{subject} {have_form} been practicing English.", f"{have_form.capitalize()} {question_subject} been practicing English?", f"{subject} {have_form} not been practicing English.", f"{subject_pt.capitalize()} tem estado praticando inglês.", f"{subject_pt.capitalize()} tem estado praticando inglês?", f"{subject_pt.capitalize()} não tem estado praticando inglês."),
        ("Past Simple", f"{subject} practiced English.", f"Did {question_subject} practice English?", f"{subject} did not practice English.", f"{subject_pt.capitalize()} praticou inglês.", f"{subject_pt.capitalize()} praticou inglês?", f"{subject_pt.capitalize()} não praticou inglês."),
        ("Past Continuous", f"{subject} {'was' if subject in ['I', 'He', 'She', 'It'] else 'were'} practicing English.", f"{'Was' if subject in ['I', 'He', 'She', 'It'] else 'Were'} {question_subject} practicing English?", f"{subject} {'was' if subject in ['I', 'He', 'She', 'It'] else 'were'} not practicing English.", f"{subject_pt.capitalize()} estava praticando inglês.", f"{subject_pt.capitalize()} estava praticando inglês?", f"{subject_pt.capitalize()} não estava praticando inglês."),
        ("Past Perfect", f"{subject} had practiced English.", f"Had {question_subject} practiced English?", f"{subject} had not practiced English.", f"{subject_pt.capitalize()} tinha praticado inglês.", f"{subject_pt.capitalize()} tinha praticado inglês?", f"{subject_pt.capitalize()} não tinha praticado inglês."),
        ("Past Perfect Continuous", f"{subject} had been practicing English.", f"Had {question_subject} been practicing English?", f"{subject} had not been practicing English.", f"{subject_pt.capitalize()} vinha praticando inglês.", f"{subject_pt.capitalize()} vinha praticando inglês?", f"{subject_pt.capitalize()} não vinha praticando inglês."),
        ("Future Simple", f"{subject} will practice English.", f"Will {question_subject} practice English?", f"{subject} will not practice English.", f"{subject_pt.capitalize()} praticará inglês.", f"{subject_pt.capitalize()} praticará inglês?", f"{subject_pt.capitalize()} não praticará inglês."),
        ("Future Continuous", f"{subject} will be practicing English.", f"Will {question_subject} be practicing English?", f"{subject} will not be practicing English.", f"{subject_pt.capitalize()} estará praticando inglês.", f"{subject_pt.capitalize()} estará praticando inglês?", f"{subject_pt.capitalize()} não estará praticando inglês."),
        ("Future Perfect", f"{subject} will have practiced English.", f"Will {question_subject} have practiced English?", f"{subject} will not have practiced English.", f"{subject_pt.capitalize()} terá praticado inglês.", f"{subject_pt.capitalize()} terá praticado inglês?", f"{subject_pt.capitalize()} não terá praticado inglês."),
        ("Future Perfect Continuous", f"{subject} will have been practicing English.", f"Will {question_subject} have been practicing English?", f"{subject} will not have been practicing English.", f"{subject_pt.capitalize()} terá estado praticando inglês.", f"{subject_pt.capitalize()} terá estado praticando inglês?", f"{subject_pt.capitalize()} não terá estado praticando inglês."),
    ]
    return [{
        "tense": tense,
        "affirmative_text": affirmative,
        "affirmative_translation": affirmative_pt,
        "question_text": question,
        "question_translation": question_pt,
        "negative_text": negative,
        "negative_translation": negative_pt,
        "example_text": affirmative,
        "example_translation": affirmative_pt,
    } for tense, affirmative, question, negative, affirmative_pt, question_pt, negative_pt in templates]
...
def groq_generate(function_name, prompt, system_instruction=None, generation_config=None, model_name=None):
    groq_keys = get_groq_api_keys()
    if not groq_keys:
        raise ValueError("GROQ_API_KEY_MISSING")

    model_name = model_name or GROQ_DEFAULT_MODEL
    temperature = (generation_config or {}).get("temperature", 0.2)
    wants_json = (generation_config or {}).get("response_mime_type") == "application/json"
    messages = []
    ...
    last_error = None
    for key_index, groq_key in enumerate(groq_keys, start=1):
        headers = {
        ...
        for allow_json_mode in ([True, False] if wants_json else [False]):
            request_payload = dict(payload)
            if not allow_json_mode:
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    if key_index < len(groq_keys):
                        st.warning(f"Groq limitou a chave {key_index}. Tentando a próxima chave Groq configurada.")
                        break
                    if retry_after:
                        st.warning(f"Groq retornou 429. Tente novamente em aproximadamente {retry_after} segundos.")
                ...
                last_error = e
                if wants_json and allow_json_mode and e.response is not None and e.response.status_code in (400, 422):
                    continue
                log_gemini_event(function_name, f"groq:{model_name}", prompt, error=f"groq key {key_index}: {e}")
                if e.response is not None and e.response.status_code == 429 and key_index < len(groq_keys):
                    break
                raise
                ...
                log_gemini_event(function_name, f"groq:{model_name}", prompt, error=f"groq key {key_index}: {e}")
                raise
    raise last_error or ValueError("GROQ_REQUEST_FAILED")
...
def gemini_generate(function_name, prompt, system_instruction=None, generation_config=None, model_name="gemini-2.5-flash", cache_enabled=True):
    ...
    if groq_keys:
        try:
            response_text = groq_generate(function_name, prompt, system_instruction, generation_config)
            if cache_enabled:
                save_gemini_cache(cache_key, function_name, prompt, response_text)
            return response_text
        except Exception as e:
            last_groq_error = e
            st.warning("Groq falhou ou bloqueou. Tentando Gemini como plano B.")
            log_gemini_event(function_name, "groq:fallback", prompt, error=f"groq primary failed: {e}")

    api_keys = get_api_keys()
    if not api_keys:
        if groq_keys:
            record_last_error(function_name, last_groq_error)
            raise last_groq_error
        raise ValueError("API_KEY_MISSING")

    last_error = None
    for key_index, api_key_candidate in enumerate(api_keys, start=1):
        genai.configure(api_key=api_key_candidate)
        for attempt in range(MAX_RETRIES_429 + 1):
            ...
            try:
                ...
                elapsed_ms = int((time.perf_counter() - started) * 1000)
                last_error = e
                log_gemini_event(function_name, model_name, prompt, elapsed_ms, error=f"key {key_index}: {e}")
                if is_rate_limit_error(e):
                    if key_index < len(api_keys):
                        st.warning(f"Limite 429 na chave {key_index}. Tentando a pr?xima chave configurada.")
                        break
                    if attempt < MAX_RETRIES_429:
                        wait_seconds = estimate_wait_seconds(attempt)
                        st.warning(f"Limite 429 detectado. Nova tentativa autom?tica em aproximadamente {wait_seconds} segundos.")
...
if active_page == "📝 Praticar Escrita & Dúvidas":
    ...
    configured_key_count = len(get_api_keys())
    st.sidebar.markdown("---")
    st.sidebar.markdown("### ?? API Key do Gemini")

    using_personal_key = bool(st.session_state.get("api_key_override"))
    if require_user_api_key() and not using_personal_key:
        st.sidebar.warning("Modo p?blico: cada usu?rio deve informar sua pr?pria Gemini API Key.")
    elif using_personal_key:
        st.sidebar.success("?? Usando chave pessoal desta sess?o")
    elif api_key_detected:
        st.sidebar.success(f"?? Usando chave da aplica??o ({configured_key_count} configurada(s))")
        st.sidebar.caption("Em app p?blico, essa chave ? compartilhada por todos os visitantes e pode bater limite rapidamente.")
    else:
        st.sidebar.warning("?? Nenhuma chave detectada!")

    with st.sidebar.expander("Usar minha pr?pria API Key", expanded=not api_key_detected):
        user_key = st.text_input(
            "Gemini API Key pessoal:",
            type="password",
            key="sidebar_key_input",
            help="Usa esta chave s? nesta sess?o e evita consumir a cota compartilhada da aplica??o."
        )
        if st.button("Salvar chave desta sess?o", use_container_width=True):
            if user_key:
                st.session_state.api_key_override = user_key
                st.rerun()
            else:
                st.warning("Cole uma API key antes de salvar.")
        if using_personal_key and st.button("Remover chave pessoal", use_container_width=True):
            st.session_state.api_key_override = ""
            st.rerun()

    groq_key_count = len(get_groq_api_keys())
    using_personal_groq_key = bool(st.session_state.get("groq_api_key_override"))
    st.sidebar.markdown("### Groq principal")
    if groq_key_count:
        st.sidebar.success(f"Groq configurado ({groq_key_count} chave(s))")
        st.sidebar.caption(f"Modelo: {GROQ_DEFAULT_MODEL}")
    else:
        st.sidebar.info("Groq ainda não configurado. O app usará Gemini como provedor principal.")

    with st.sidebar.expander("Usar Groq API Key", expanded=False):
        groq_user_key = st.text_input(
            "Groq API Key:",
            type="password",
            key="groq_sidebar_key_input",
            help="Usa Groq como provedor principal. Gemini fica como plano B."
        )
        if st.button("Salvar chave Groq desta sessão", use_container_width=True):
            if groq_user_key:
                st.session_state.groq_api_key_override = groq_user_key
                st.rerun()
            else:
                st.warning("Cole uma Groq API key antes de salvar.")
        if using_personal_groq_key and st.button("Remover chave Groq", use_container_width=True):
...
with st.sidebar.expander("Painel administrativo Gemini", expanded=False):
    ...
    if st.session_state.gemini_session_history:
        ...
        for event in st.session_state.gemini_session_history[:5]:
            label = "cache" if event.get("cache_hit") else "dev" if event.get("dev_mode") else event.get("provider", "gemini")
